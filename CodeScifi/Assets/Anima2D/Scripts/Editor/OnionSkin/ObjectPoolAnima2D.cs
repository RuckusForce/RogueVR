...
namespace Anima2D.Pool
{
	...
	public abstract class ObjectPoolAnima2D< T >
	{
		...
		protected ObjectPoolAnima2D()
		{
			m_AvaliableObject = new List<T>();
			m_DispatchedObjects = new List<T>();
		}

		public ObjectPoolAnima2D(ICreationPolicy<T> _creationPolicy) : this()
		{
	...
	public class DefaultObjectPool<T> : ObjectPoolAnima2D<T> where T : new ()
	{
